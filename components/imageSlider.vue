<template>
    ...
    <div class="overflow-hidden relative  m-auto h-full" @touchstart="handleTouchStart($event)"
        ...
        <!-- wrapper -->
        <div class=" bg-white rounded-lg border-[#6082B6] border-2 ">
            <div class="flex transition-transform duration-[0.3s] ease-[ease-in-out] relative" ref="wrapper">
                <div class="min-w-full p-10 slides cursor-pointer" v-for="(slide, index) in thumbnails" v-if="thumbnails"
                    ...
                    <div v-if="slide.img" class="w-fit mx-auto relative"
                        ...
                        <div class="absolute  w-60 h-60 xl:block hidden bg-[rgba(95,221,241,0.4)]"
                        ...
                        <img :src="slide.img" class="sm:max-h-[480px] max-h-[320px] h-full" :alt="slide" ref="imgs"
                            @mousemove="zoomEffect(index), moveLens($event, index)"
            ...
            <div class="flex justify-center items-center mb-2">
                <div v-for="(slideThumb, index ) in thumbnails" class="md:mx-2 mx-1 p-2 cursor-pointer rounded border-2"
                    :class="currentIndex == index ? 'border-[#6082B6]' : 'border-transparent'" :key="index"
        ...
        <button @click="prevSlide()"
            class="absolute -translate-y-[100%] text-lg cursor-pointer text-[#333] border-2 border-[#6082B6] top-2/4 rounded-full p-2 font-bold left-2.5">
            <img src="../assets/svgItems/leftArrow.svg" class="h-5 w-5" alt="Left-arrow">
        </button>
        <button @click="nextSlide()"
            class="absolute -translate-y-[100%] text-lg cursor-pointer text-[#333] border-2 border-[#6082B6] top-2/4 rounded-full p-2 font-bold right-2.5">
            <img src="../assets/svgItems/rightArrow.svg" class="h-5 w-5" alt="Left-arrow">
        </button>

        
        <div class="text-center mt-5">
            <div class="inline-block w-2.5 h-2.5 cursor-pointer mx-[5px] my-0 rounded-[50%]"
                :class="currentIndex == index ? 'bg-[#6082B6] border' : 'bg-transparent border border-slate-500'"
